...
int main(int argc, char** argv) {
    ...
    init_params.depth_mode = sl::DEPTH_MODE::ULTRA;   // 深度模式：最高精度
    ...
    KalmanFilter1D kf_t0x(0.0001,0.003,0.0),kf_t0y(0.0001,0.003,0.0),kf_t0z(0.0001,0.003,0.0);
    KalmanFilter1D kf_r0x(0.00001,0.05,0.0),kf_r0y(0.00001,0.05,0.0),kf_r0z(0.00001,0.05,0.0);
    KalmanFilter1D kf_t1x(0.0001,0.003,0.0),kf_t1y(0.0001,0.003,0.0),kf_t1z(0.0001,0.003,0.0);
    KalmanFilter1D kf_r1x(0.00001,0.05,0.0),kf_r1y(0.00001,0.05,0.0),kf_r1z(0.00001,0.05,0.0);
    KalmanFilter1D kf_t2x(0.0001,0.003,0.0),kf_t2y(0.0001,0.003,0.0),kf_t2z(0.0001,0.003,0.0);
    KalmanFilter1D kf_r2x(0.00001,0.05,0.0),kf_r2y(0.00001,0.05,0.0),kf_r2z(0.00001,0.05,0.0);
        ...
        if (zed.grab() == sl::ERROR_CODE::SUCCESS) {
            ...
            if (!ids.empty()) {
                ...
                if (id0_found && id2_found) {
                    ...
                    // Apply both translation and rotation corrections
                    Vec3d err_t(t1r.at<double>(0)-g_ref_t[0], t1r.at<double>(1)-g_ref_t[1], t1r.at<double>(2)-g_ref_t[2]);
                    Mat R1r=rvecToMatrix(id1_rvec); Mat R_err=R0r.t()*R1r*g_ref_R.t();
                    t_rel = R_err.t() * t_rel - Mat(err_t);
                    ...
                    ss << "L:\[DEPTH] " << sqrt(corr_x*corr_x+corr_y*corr_y+corr_z*corr_z)*1000 << "mm"
                       << (assembly_moving ? " [MOV]" : "");
            ...
            stringstream simu; simu<<fixed<<setprecision(3)<<"IMU a:"<<acc<<" g:"<<gyr;
            putText(frame_left,simu.str(),Point(20,img_h-20),FONT_HERSHEY_SIMPLEX,0.4,Scalar(0,255,0),1);
